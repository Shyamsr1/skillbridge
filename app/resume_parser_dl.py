...
# --- NER-based Skills using HuggingFace ---

def extract_skills_bert(text):

    entities = ner_pipe(text)

    return [e['word'] for e in entities if e['entity_group'] in ["ORG", "PER", "MISC", "LOC"]]
...
def parse_resume_unified(file_path, threshold=0.75):
    ...
    semantic_skills = semantic_match_skills(skills_bert, skill_list)
